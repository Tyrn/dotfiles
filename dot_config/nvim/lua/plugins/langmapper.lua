...
return {
  ...
  },
  {
    ...
    optional = true,
    config = function(_, _)
      -- Snacks is guaranteed to be loaded here by Lazy
      local Snacks = _G.Snacks
      -- require "snacks" -- either works; _G.Snacks is set by snacks.nvim【turn8fetch0】
      local translate_key = require("langmapper.utils").translate_keycode
      local normkey_orig = Snacks.util.normkey
      Snacks.util.normkey = function(key)
        if key then key = translate_key(key, "default", "ru") end
